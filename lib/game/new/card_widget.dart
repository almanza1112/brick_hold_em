...
class CardWidget extends ConsumerWidget {
  ...
  /// Set to true if the card is already on the table.
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return GestureDetector(
      onTap: () async {
        if (isOnTable) {
          // Untap logic: remove the card from tappedCards and add it back to the hand.
          ref.read(tappedCardsProvider.notifier).removeCard(cardKey);
          ref.read(handProvider.notifier).addCard(cardKey);
        } else {
          // Tapping a card from your hand to move it to the table.
          if (ref.read(isPlayersTurnComputedProvider)) {
            // If there is room on the table, add the card to tapped cards and remove it from the hand.
                          ...
                          children: [
                            ...
                            ElevatedButton(
                              ...
                              onPressed: () {
                                ...
                                // Add updated card to tapped cards.
                                ref
                                    .read(tappedCardsProvider.notifier)
                                    .addCard(updatedCard);
                                // Remove card from hand.
                                ref
                                    .read(handProvider.notifier)
                                    .removeCard(cardKey);
